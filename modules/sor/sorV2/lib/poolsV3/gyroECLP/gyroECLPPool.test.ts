...
import { GyroECLPPool } from './gyroECLPPool';
...
// TODO: refactor tests to use GyroECLPPool

describe('SOR V3 Stable Pool Tests', () => {
    let amp: string;
    let scalingFactors: bigint[];
    let aggregateSwapFee: bigint;
    let stablePool: GyroECLPPool;
    let stablePoolWithHook: GyroECLPPool;
    ...
    beforeAll(() => {
        ...
        stablePool = GyroECLPPool.fromPrismaPool(stablePrismaPool, []);
        ...
        stablePoolWithHook = GyroECLPPool.fromPrismaPool(stablePrismaPoolWithHook, []);
    });
