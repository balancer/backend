...
export class MorphoHyperevm implements YbAprHandler {
    constructor(private config: YbAprConfig['morphoVaultHyperevm']) {}
    ...
    async getAprs() {
        ...
        try {
            ...
            this.config!.forEach((vaultAddress) => {
            ...
            // create vault data
            for (const vaultAddress of this.config!) {
                const supplyQueueLength = supplyQueueLengthResult[`${vaultAddress}`].supplyQueueLength;
                const supplyQueue: {
                    marketAddress: string;
                    totalSupplyShares: bigint;
            ...
            vaultData.forEach((vault) => {
                ...
                allAprs[vault.address.toLowerCase()] = {
                    apr: supplyAPY / 100,
                    isIbYield,
                };
            });
