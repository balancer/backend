...
export const addPools = async (subgraphPools: V3JoinedSubgraphPool[], chain: Chain, blockNumber: number) => {
    ...
    const prices = await prisma.prismaTokenCurrentPrice
        .findMany({
            where: {
                chain: chain,
                tokenAddress: { in: data.flatMap((item) => item.tokens).map((token) => token.address) },
            },
        })
        .then((priceData) => Object.fromEntries(priceData.map((price) => [price.tokenAddress, price.price])));

    const withUsd = data.map((item) => enrichPoolUpsertsUsd(item, prices));
