...
import { OnChainRootGauges, RootGauge, hardcodedGauges } from './root-gauges.onchain';
...
//     const onchainRootGauges = await fetchRootGauges(testHttpClient);
...
it.only('Full flow', async () => {
    ...
    // onchainRootAddresses = ['0x9fb8312cedfb9b35364ff06311b429a2f4cdf422'];

    await service.deleteRootGauges();

    const chunks = chunk(onchainRootAddresses, 50);

    for (const addressChunk of chunks) {
        try {
            await saveChunk(addressChunk);
        } catch (error) {
    ...
    async function saveChunk(addressChunk: string[]) {
        const rootGauges = await service.fetchRootGauges2(testHttpClient, addressChunk);

        // const rootGaugesByAddress = keyBy(rootGauges, 'gaugeAddress');

        // console.log(rootGaugesByAddress['0x9fb8312cedfb9b35364ff06311b429a2f4cdf422']);
        // //TODO: save hardcoded gauges later
        const cleanRootGauges = rootGauges.filter((gauge) => !hardcodedGauges.includes(gauge.gaugeAddress));
        // console.log(cleanRootGauges.length);
        await service.saveRootGauges(cleanRootGauges);
    }
}, 10_000);
